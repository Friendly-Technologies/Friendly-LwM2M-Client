...
if [ ! -f "$BUILD_DIR/include/coap3/coap.h" ]; then
    ...
    cmake .. -DCMAKE_BUILD_TYPE=MinSizeRel -DBUILD_SHARED_LIBS=ON -DENABLE_DTLS=ON -DWITH_TINYDTLS=ON -DDTLS_BACKEND=tinydtls -DENABLE_SERVER_MODE=OFF -DENABLE_EXAMPLES=OFF -DENABLE_DOCS=OFF >> "$LOG_FILE" 2>&1
