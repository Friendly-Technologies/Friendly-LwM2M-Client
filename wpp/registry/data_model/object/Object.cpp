...
/* ------------- Lwm2m core callback ------------- */
uint8_t Object::read_clb(lwm2m_context_t * contextP, ID_T instanceId, int * numDataP, lwm2m_data_t ** dataArrayP, lwm2m_object_t * objectP) {
	...
	return obj->instance(instanceId)->read(numDataP, dataArrayP);
}

uint8_t Object::write_clb(lwm2m_context_t * contextP, ID_T instanceId, int numData, lwm2m_data_t * dataArray, lwm2m_object_t * objectP, lwm2m_write_type_t writeType) {
	Object *obj = (Object *)objectP->userData;
    WPP_LOGD_ARG(TAG_WPP_OBJ, "wakaama write %d:%d", obj->getObjectID(), instanceId);
	if (!obj->isInstanceExist(instanceId)) return COAP_404_NOT_FOUND;
	return obj->instance(instanceId)->write(numData, dataArray, writeType);
}

uint8_t Object::execute_clb(lwm2m_context_t * contextP, ID_T instanceId, ID_T resId, uint8_t * buffer, int length, lwm2m_object_t * objectP) {
	Object *obj = (Object *)objectP->userData;
    WPP_LOGD_ARG(TAG_WPP_OBJ, "wakaama execute %d:%d", obj->getObjectID(), instanceId);
	if (!obj->isInstanceExist(instanceId)) return COAP_404_NOT_FOUND;
	return obj->instance(instanceId)->execute(resId, buffer, length);
}

uint8_t Object::discover_clb(lwm2m_context_t * contextP, ID_T instanceId, int * numDataP, lwm2m_data_t ** dataArrayP, lwm2m_object_t * objectP) {
	Object *obj = (Object *)objectP->userData;
    WPP_LOGD_ARG(TAG_WPP_OBJ, "wakaama discover %d:%d", obj->getObjectID(), instanceId);
	if (!obj->isInstanceExist(instanceId)) return COAP_404_NOT_FOUND;
	return obj->instance(instanceId)->discover(numDataP, dataArrayP);
}

uint8_t Object::create_clb(lwm2m_context_t * contextP, ID_T instanceId, int numData, lwm2m_data_t * dataArray, lwm2m_object_t * objectP) {
	Object *obj = (Object *)objectP->userData;
	...
	obj->observerNotify(*obj, instanceId, InstOp::CREATE);
...
uint8_t Object::delete_clb(lwm2m_context_t * contextP, ID_T instanceId, lwm2m_object_t * objectP) {
	Object *obj = (Object *)objectP->userData;
    WPP_LOGD_ARG(TAG_WPP_OBJ, "wakaama delete %d:%d", obj->getObjectID(), instanceId);
	if (!obj->isInstanceExist(instanceId)) return COAP_404_NOT_FOUND;
	// Notify user about deleting instance
	obj->observerNotify(*obj, instanceId, InstOp::DELETE);
