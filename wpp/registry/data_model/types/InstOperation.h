...
public:
	InstOperation(uint8_t flags = TYPE::NONE): _flags(flags) {}
    inline bool isSupported(TYPE type) const { return _flags & type; };
    inline bool isCompatible(const InstOperation& operation) const { return (_flags & operation._flags) == _flags; };
